...
func GenerateGenesis(logger *logrus.Logger) *cobra.Command {
	...
	var txCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			extraNodesCount, err := cmd.Flags().GetInt("extraNodes")
			if err != nil {
				logger.Printf("%s", err)
			}
			if args[0] == "generate" {
				generateFiles(logger, withDbLastState, dbPath, extraNodesCount)
			} else {
	...
	txCmd.Flags().IntVarP(&extraNodesCount, "extraNodes", "n", 0,
...
func generateFiles(logger *logrus.Logger, withDbLastState bool, dbPath string, extraNodesCount int) {
	var (
