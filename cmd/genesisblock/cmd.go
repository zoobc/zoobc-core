...
func generateRandomGenesisEntry(accountAddress string) genesisEntry {
	...
	nodePublicKeyStr, _ := address.EncodeZbcID(model.PrefixConstant_ZNK.String(), nodePublicKey)
...
func getDbLastState(dbPath string) (bcEntries []genesisEntry, err error) {
	...
	for _, acc := range accountBalances {
		...
		if len(nodeRegistrations) > 0 {
			...
			bcEntry.NodePublicKey, _ = address.EncodeZbcID(model.PrefixConstant_ZNK.String(), nr.NodePublicKey)
...
func validateGenesisFile(genesisEntries []genesisEntry) bool {
	...
	for _, genesisEntry := range genesisEntries {
		...
		computedPbKeyStr, _ := address.EncodeZbcID(model.PrefixConstant_ZNK.String(), computedPbKey)
