...
func TestMempoolService_ProcessTransactionBytesToMempool(t *testing.T) {
	...
	type conditions struct {
		...
		parseTransactionResult *model.Transaction
		parseTransactionError  error
		// actionSwitcherGetTransactionTypeResult transaction.TypeAction
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mps := &MempoolService{
				...
				BlockTxCached:    tt.conditions.blockTxCached,
			...
			err := mps.ProcessTransactionBytesToMempool(tt.args.mempoolTx)
			if (err != nil) != tt.wantErr {
				t.Errorf("ProcessTransactionBytesToMempool() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
