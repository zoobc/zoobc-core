...
func TestSnapshotMainBlockService_calculateBulkSize(t *testing.T) {
	type fields struct {
		SnapshotPath                  string
		chainType                     chaintype.ChainType
		TransactionUtil               transaction.UtilInterface
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotRecordsPerPeriod, gotRounds, gotRemaining := ss.calculateBulkSize(tt.args.totalFields, tt.args.totalRecords)
