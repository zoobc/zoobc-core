...
func (tu *Util) ParseTransactionBytes(transactionBytes []byte, sign bool) (*model.Transaction, error) {
	...
	senderAccountAddress := tu.ReadAccountAddress(util.ConvertBytesToUint32(senderAccountAddressLength), buffer)
	recipientAccountAddressLength, err := util.ReadTransactionBytes(buffer, int(constant.AccountAddressLength))
	if err != nil {
		return nil, err
	}
	recipientAccountAddress := tu.ReadAccountAddress(util.ConvertBytesToUint32(recipientAccountAddressLength), buffer)
