...
*/
func (tx *SetupAccountDataset) ApplyUnconfirmed(applyInCache bool) error {
	if applyInCache {
...
*/
func (tx *SetupAccountDataset) EscrowApplyUnconfirmed(applyInCache bool) error {
	var addedSpendable = -(tx.Fee + tx.Escrow.GetCommission())
	if applyInCache {
		return tx.AccountBalanceHelper.AddAccountSpendableBalanceInCache(
			tx.SenderAddress,
			addedSpendable,
		)
	}
