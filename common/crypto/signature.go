...
type (
	...
	SignatureInterface interface {
		...
		SignHDWallet(payload []byte, signatureType model.SignatureType, seed string, PrivateKey []byte) ([]byte, error)
...
func (*Signature) SignHDWallet(payload []byte, signatureType model.SignatureType, seed string, PrivateKey []byte) ([]byte, error) {
	buffer := bytes.NewBuffer([]byte{})
	buffer.Write(util.ConvertUint32ToBytes(uint32(signatureType)))
	switch signatureType {
	case model.SignatureType_DefaultSignature:
