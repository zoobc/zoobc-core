...
func TestAccountBalanceQuery_SelectDataForSnapshot(t *testing.T) {
	t.Run("SelectDataForSnapshot", func(t *testing.T) {
		...
		wantQ := "SELECT account_address,block_height,spendable_balance,balance,pop_revenue," +
			"latest FROM account_balance WHERE block_height >= 0 AND block_height <= 10 AND (block_height || '_' || account_address) IN (" +
			"SELECT (MAX(block_height) || '_' || account_address) as con FROM account_balance GROUP BY account_address) ORDER BY block_height"
		if q != wantQ {
			t.Errorf("query returned wrong: get: %s\nwant: %s", q, wantQ)
		}
	})
